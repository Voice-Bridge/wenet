...
class SpeechToText:
    ...
    def load_model(cls, api_name: str) -> 'SpeechToText':
        ...
        json_file_path = os.path.join(current_dir, 'xunfei_apikey.json')
        try:
            with open(json_file_path, 'r', encoding='utf-8') as f:
                config = json.load(f)
        except FileNotFoundError:
            print(f"Error: 文件 {json_file_path} 未找到。")
        except json.JSONDecodeError as e:
            print(f"Error: 解析 JSON 文件时出错: {e}")
        api_credentials = {
            'xunfei_api': {
    ...
    def transcribe(self, audio_file: str, confidence: float = 0.0) -> dict:
        wav_file = 'D://sample_audios/47_112.225.101.110_20241119074051_博物馆.wav'
